...
async function refreshTuyaEntityList() {
  const list = $("tuyaEntityList");
  if (!list) return;
  try {
    const devices = await apiFetch("/api/tuya/devices");
    list.innerHTML = "";
    (devices || []).forEach((d) => {
      const codes = d.codes && d.codes.length ? d.codes : [""];
      codes.forEach((code) => {
        const opt = document.createElement("option");
        opt.value = code ? `tuya.${d.id}.${code}` : `tuya.${d.id}`;
        opt.label = `${d.name}${code ? " · " + code : ""}`;
        list.appendChild(opt);
      });
    });
  } catch (e) {
    console.debug("tuya devices list", e);
  }
}

async function loadTuyaSettingsForm() {
  try {
    const cfg = await apiFetch("/api/tuya/config");
    if ($("tuyaAccessId")) $("tuyaAccessId").value = cfg.access_id || "";
    if ($("tuyaUid")) $("tuyaUid").value = cfg.uid || "";
    if ($("tuyaRegion")) $("tuyaRegion").value = cfg.region || "eu";
    if ($("tuyaPreferLocal")) $("tuyaPreferLocal").checked = cfg.prefer_local !== false;
    ...
    console.debug("tuya config", e);
  }
}

async function saveTuyaSettings() {
  const body = {
    access_id: $("tuyaAccessId")?.value?.trim() || "",
    uid: $("tuyaUid")?.value?.trim() || "",
    region: $("tuyaRegion")?.value || "eu",
    prefer_local: !!$("tuyaPreferLocal")?.checked,
  };
  const secret = $("tuyaAccessSecret")?.value?.trim();
  if (secret) body.access_secret = secret;
  const res = await apiFetch("/api/tuya/config", {
    method: "POST",
    body: JSON.stringify(body),
  });
  await loadTuyaSettingsForm();
  await refreshTuyaEntityList();
  await fetchEntityStates();
  if (res.demo) toast("Modo demo activo (faltan claves o falló el login)", "warn");
  else if (res.connected) toast(`Tuya conectado (${res.device_count} dispositivos)`, "success");
  else toast(res.last_error || "No se pudo conectar a Tuya", "error");
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const openTuya = $("btnOpenTuyaSettings");
  if (openTuya) openTuya.addEventListener("click", () => { loadTuyaSettingsForm(); show("tuyaSettingsModal"); });
  const cancelTuya = $("btnCancelTuyaSettings");
  if (cancelTuya) cancelTuya.addEventListener("click", () => hide("tuyaSettingsModal"));
  const saveTuya = $("btnSaveTuyaSettings");
  if (saveTuya) saveTuya.addEventListener("click", async () => {
    try { await saveTuyaSettings(); hide("tuyaSettingsModal"); }
    catch (err) { toast(`Tuya: ${err.message}`, "error"); }
  });
  const syncTuya = $("btnSyncTuya");
  if (syncTuya) syncTuya.addEventListener("click", async () => {
    try {
      await apiFetch("/api/tuya/sync", { method: "POST", body: "{}" });
      await loadTuyaSettingsForm();
      await refreshTuyaEntityList();
      await fetchEntityStates();
      toast("Dispositivos Tuya sincronizados", "success");
    } catch (err) { toast(`Sync: ${err.message}`, "error"); }
  });
  const tuyaModal = $("tuyaSettingsModal");
  if (tuyaModal) tuyaModal.addEventListener("click", (e) => { if (e.target === tuyaModal) hide(tuyaModal); });
...
async function exportConfiguration() {
  try {
    ...
    a.href = url;
    a.download = `hadomotics_backup_${new Date().toISOString().slice(0,10)}.json`;
...
async function importConfiguration(file) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      toast("Configuration imported successfully! Reloading...", "success");
